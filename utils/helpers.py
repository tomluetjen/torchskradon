...
def sinogram_circle_to_square(sinogram):
    ...
    return torch.nn.functional.pad(sinogram, (0, 0, pad_width[0], pad_width[1]), mode='constant', value=0)
...
def get_fourier_filter(size, filter_name, device=None, dtype=torch.float32):
    ...
    if filter_name == "ramp":
        ...
        freq = torch.linspace(0, torch.pi, size, device=device, dtype=dtype)
