...
sinograms = torchskradon(images, theta=theta, circle=False)
...
class RadonModel(torch.nn.Module):
    def __init__(self, theta):
        super(RadonModel, self).__init__()
        self.theta = theta

    def forward(self, x):
        return torchskradon(x, theta=self.theta, circle=False)
